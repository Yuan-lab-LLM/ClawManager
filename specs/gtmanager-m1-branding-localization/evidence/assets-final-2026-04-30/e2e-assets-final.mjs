...
try {
  ...
  result.logo = {
    ...
    strictDecoderCaveat: 'Still unresolved per deployment-refresh-assets evidence; browser load is the acceptance signal recorded here.',
    passed:
  ...
  const logoImages = await page.locator('img').evaluateAll((images) =>
    images.map((image) => ({
  ...
  result.loginBeforeSubmit = {
    ...
    passed:
      ...
      logoImages.some((image) => image.src === '/gtmanager-logo.png' || image.currentSrc?.endsWith('/gtmanager-logo.png')) &&
  ...
  if (!result.loginBeforeSubmit.logoImages.some((image) => image.src === '/gtmanager-logo.png' || image.currentSrc?.endsWith('/gtmanager-logo.png'))) {
    result.failures.push('Clean-context /login did not expose an img logo using /gtmanager-logo.png.');
  }
...
await fs.writeFile(
  path.join(evidenceDir, 'playwright-assets-final-result.json'),
...
console.log(JSON.stringify({
  ...
  resultJson: path.join(evidenceDir, 'playwright-assets-final-result.json'),
  screenshotsDir,
  logsDir,
}, null, 2));
