...
5. **测试驱动 & 可复现**
   ...
   - CLI 测试必须设置超时，避免卡死。
   - 最终验收必须通过端到端测试：构建成功、参数检查通过、单测/集成测试通过，只能作为前置证据，不能单独宣称功能“已通过”。
   - 端到端测试可以是自动化测试（例如 Playwright 覆盖真实 UI/API/运行时链路），也可以是明确通知人工测试并由测试人反馈结果；未完成端到端验证时，只能标记为“单测/构建通过，E2E 待验证”。
...
2. **质量门槛**
   - 测试策略：
     ...
     - 前端：当前真实脚本为 `npm run lint` 与 `npm run build`；只有在批准引入具体前端测试工具后才要求 `npm test`
     - 部署：`kubectl apply --dry-run=client` 验证 YAML 合法性
     - 端到端：关键功能必须跑通真实用户/控制面/运行时链路；可使用 Playwright 自动化，或由负责人手动验证并记录步骤、环境、结果
   - 验证方式落盘：所有测试命令写入 `longterm/workspace/init.sh`
   - 通过判定：没有端到端验证证据的功能不得标记为 `passes: true`，不得在总结、PR、交付说明中写作“完成/通过/已验收”
...
**Version**: 1.0.1 | **Ratified**: 2026-04-14 | **Last Amended**: 2026-04-29
